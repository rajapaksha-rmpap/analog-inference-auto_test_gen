...
# getting a list of all the input tests 
file_list = os.scandir(in_json_path)
in_test_list = [file.name.split(".")[0] for file in file_list if file.name.endswith(".json")]

if len(sys.argv) == 1: 
    # if no test case is specified, run all the input test casees
    run_all = input("do you want to run all the tests in %s/ [Y/n]? :- " %(os.path.abspath(in_json_path)))
...
for test_name in test_list: 
    if test_name not in in_test_list: 
        print("specified test '%s' does not exist in %s/ \n" %(test_name, os.path.abspath(in_json_path)))
        continue 
    ...
    # read other sim.json specs if they exist in input json and move them to sim_json 
    ...
    print("creating %s-spec.json and %s-sim.json files was successful!\n" %(test_name, test_name))
    success += 1

print("%d out of %d: sim and spec json fies were successfully auto generated" %(success, len(test_list)))
